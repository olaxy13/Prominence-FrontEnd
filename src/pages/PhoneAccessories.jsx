...
import { fetchPhoneAccessories } from "../services/data";
...
const PhoneAccessories = () => {
  ...
  const phoneAccessoriesData = data?.data || [];
  const brands = Array.from(new Set(phoneAccessoriesData.map((l) => l.brand)));
  ...
  const filteredPhoneAccessories =
    selectedBrands.length === 0
      ? phoneAccessoriesData
      : phoneAccessoriesData.filter((accessories) =>
    ...
    <div className="flex flex-wrap md:flex-nowrap gap-8 p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      <main className="flex-1">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredPhoneAccessories.map((accessories) => (
            <div
